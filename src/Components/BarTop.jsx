    ...
    if (token) {
      async function fetchFriends() {
        try {
          ...
          console.log(data); // Log the data to ensure it's an array
          setFriends(data);
        } catch (error) {
          console.error("Error fetching friends:", error);
          toast.error('Error fetching friends!');  // Show an error message if the fetch fails
        }
      }

      fetchFriends();
    }
  }, [token]);  // Add token as a dependency to ensure it runs when the token changes
