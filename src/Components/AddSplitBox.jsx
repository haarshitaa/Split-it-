...
//             const share  = parseFloat(formData.amount) / formData.participants.length;
...
import { useState, useEffect } from 'react';
import { Card, CardMedia,CardContent, Button, TextField ,Autocomplete} from '@mui/material';
import axios from 'axios';
...
export function AddSplitBox({ description, image, currency, amount, note, participants, onSubmit }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if(!formData.amount || formData.participants.length === 0){
            alert('Amount and participants are required');
            return;
        }
        onSubmit(formData); 
    };
